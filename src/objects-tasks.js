...
function removeProperties(obj, keys) {
  const answer = obj;
  keys.forEach((key) => delete answer[key]);
...
function compareObjects(obj1, obj2) {
  ...
  values1.forEach((val1, index) => {
    if (val1 !== values2[index]) isEqual = false;
  });
...
function makeWord(lettersObject) {
  ...
  indexes.forEach((array, index) => {
    const { length } = array;

    for (let i = 0; i < length; i += 1) {
      answer[array[i]] = letters[index];
    }
  });
...
function Rectangle(width, height) {
  ...
  const r = new RectangleObj(width, height);
  return r;
}
...
function group(/* array, keySelector, valueSelector */) {
  ...
  // const values = [];
  // const map = new Map();
  // array.forEach((object) => keys.push(keySelector(object)));
  // array.forEach((object) => values.push(valueSelector(object)));
  // keys.forEach((key) => map.set(key));
  // console.log(keys);
  // console.log(values);
  // array.forEach((object) => console.log(Object.entries(object)));
